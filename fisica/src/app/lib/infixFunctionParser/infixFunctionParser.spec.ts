import {
  BinaryOperation,
  FunctionExpression, GroupCloseToken, GroupOpenToken, MinusToken,
  MultiplyToken,
  SumToken,
  TerminalToken,
  TokenAnalyzer,
...
describe('tokens to prefix notation transpiler', () => {
  ...
  it('grouping tokens {(a + b) * c}', () => {
    const tokens = [new GroupOpenToken(), new TerminalToken('a'), new SumToken(), new TerminalToken('b'), new GroupCloseToken(), new MultiplyToken(), new TerminalToken('c')];
    const expression = TokenAnalyzer.analyze(tokens);
    expect(expression.toPrefixNotation()).toEqual(['*', '+', 'a', 'b', 'c']);
  });
});
